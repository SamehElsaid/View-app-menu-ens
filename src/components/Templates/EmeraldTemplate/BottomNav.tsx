...
interface Props {
  ...
  return (
    <nav
      className="fixed bottom-0 inset-x-0 z-40 md:hidden bg-white/[0.94] backdrop-blur-[20px] shadow-[0_-4px_24px_rgba(76,17,33,0.1)] px-2 pb-safe"
          ...
          return (
            <button
              ...
              className={`
                ...
                ${
                  isActive
                    ? "bg-gradient-to-br from-[#4c1121] to-[#9b2545] text-white shadow-[0_4px_20px_rgba(155,37,69,0.4)]"
                    : "text-stone-500 hover:bg-[#fef1f5] hover:text-[#62162a]"
                }
              `}
            >
              <span className="text-sm leading-none">{cat.name}</span>
              <span>{locale === "ar" ? cat.nameAr || cat.name : cat.nameEn || cat.name}</span>
            </button>
          );
        })}
      </div>
