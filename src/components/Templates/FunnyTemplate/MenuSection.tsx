      ...
      {tabs.map((cat) => {
        ...
        return (
          <button
            ...
            ${
              isActive
                ? "text-white shadow-lg ring-2 ring-white/45"
                : "bg-white text-stone-800 shadow-md border border-white/90 hover:bg-white hover:shadow-lg"
            }
          `}
...
function FunnyMenuCard({
  ...
  const cardHoverShadow = "0 16px 48px rgba(0,0,0,0.28)";
  ...
  return (
    ...
    >
      <div className="relative h-52 overflow-hidden bg-black/10">
        ...
        />
        {badgeText && (
          <span className="absolute top-3 start-3 text-[11px] font-sans font-700 px-2.5 py-1 rounded-full tracking-wider uppercase bg-white/95 text-stone-800 shadow-md">
            {badgeText}
          </span>
        )}
      </div>
      ...
      >
        <div className="relative shrink-0 w-full aspect-[4/3] min-h-[220px] sm:min-h-[280px] overflow-hidden">
          ...
          {dish.discountPercent ? (
            <span className="absolute top-3 start-3 rounded-full px-3 py-1.5 text-[11px] font-sans font-700 uppercase tracking-wider text-stone-900 bg-white/95 shadow-lg">
